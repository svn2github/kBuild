...
CC = gcc
CFLAGS = -I. -I../kLib/Generic/include \
         -DMACHINE=\"ibmos2\" -DMACHINE_ARCH=\"x86\" -DMACHINE_CPU=\"386\" \
...
$(LSTOBJS): lst.lib/$(@B).c list.h lst.h
    @mkdir -p $(OBJDIR)
    $(CC) -c $(CFLAGS) -o $(OBJDIR)/$(@B).o lst.lib/$(@B).c

$(BASEOBJS): $(@B).c list.h lst.h make.h
    @mkdir -p $(OBJDIR)
    $(CC) -c $(CFLAGS) $(@B).c -o $(OBJDIR)/$(@B).o

$(INCOBJS): include/$(@B).c list.h lst.h make.h
    @mkdir -p $(OBJDIR)
    $(CC) -c $(CFLAGS) -o $(OBJDIR)/$(@B).o include/$(@B).c
