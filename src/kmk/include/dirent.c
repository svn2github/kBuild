...
getFSType(const char *path)
{
	...
	infolen = sizeof info;
#if OS2VER >= 2
	if (DosQueryFSAttach(drive, 0, FSAIL_QUERYNAME, (PFSQBUFFER2)(PVOID)&info[0], &infolen))
		return 0;
...
#else
	if (DosQFSAttach((PSZ)drive, 0, FSAIL_QUERYNAME, (PFSQBUFFER2)(PVOID)&info[0], &infolen, 0))
		return 0;
	if (infolen >= 9) {
...
int readdir_r(DIR *dir, struct dirent *entry, struct dirent **result)
{
    *result = readdir(dir);
    if (*result && entry)
        memcpy(entry, *result, sizeof(*entry));
    return *result != NULL ? 0 : 9;
}
